...
def do_something2(command: str) -> None:
    ...
    match command.split():
        ...
        case ['quit' | 'exit' | 'leave' | 'stop' | 'end' | 'terminate' | 'kill' | 'bye', *rest]:
            print('bye')
            if 'no' in rest:
...
def do_something3(command: str) -> None:
    ...
    match command.split():
        ...
        case ['green', fruit]:
            # '\033[0;32m' is the escape sequence for green color
            print(f'\033[0;32m{fruit}')
        case ['quit' | 'exit' | 'leave' | 'stop' | 'end' | 'terminate' | 'kill' | 'bye', *rest] if 'no' in rest:
            print('tricky move,next time be sure about your choice')
        case ['quit' | 'exit' | 'leave' | 'stop' | 'end' | 'terminate' | 'kill' | 'bye']:
            print('bye')
            quit()
        case other:
            print(f'we don\'t have {other} ')
...
def do_something4(command: Command) -> None:
    match command:
        ...
        case Command('quit' | 'kill' | 'exit' | 'bye'):
            ...
            quit()
        case other:
            print(f'we don\'t need this -> {other.arguments} because this is -> {other.command!r} unknown command  , mate')
